...
function TimerComponent() {
  ...
  return (
    <div className="relative p-6 rounded-3xl flex-1 text-white min-w-72 overflow-hidden shadow-lg">
      {/* Background Layers Crossfading */}
      <motion.div
        className="absolute inset-0 rounded-3xl bg-gradient-to-br from-purple-700 to-indigo-800 transition-opacity"
        initial={{ opacity: isBreakMode ? 0 : 1 }}
        animate={{ opacity: isBreakMode ? 0 : 1 }}
        transition={{ duration: 0.5 }}
      />
      <motion.div
        className="absolute inset-0 rounded-3xl bg-gradient-to-br from-emerald-700 to-sky-800 transition-opacity"
        initial={{ opacity: isBreakMode ? 1 : 0 }}
        animate={{ opacity: isBreakMode ? 1 : 0 }}
        transition={{ duration: 0.5 }}
      />
      ...
      {/* Main Content */}
      <div className="relative z-10">
        <nav className="flex mb-4 items-center gap-6 justify-center">
          <div className="relative">
            <button
              onClick={() => setIsBreakMode(false)}
              className={`text-xl transition-colors duration-300 pb-1 ${
